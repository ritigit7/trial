...
def main():
    ...
    # if start_detection:
    #     rtsp_url = f'rtsp://{username}:{password}@{ip_address}'
    #     st.info("Connecting to camera...")atio


    #     video_placeholder = st.empty()
    #     if selected_camera=="Parking Road" or selected_camera=="Main Gate":
    #         try:
    #             for frame in number_plate_detection(rtsp_url):
    #                 video_placeholder.image(frame, channels="BGR")
    #         except Exception as e:
    #             st.error(f"Error: {e}")
        ...
        # elif selected_camera == "Main Gate":
        #     try:
        #         for ppe_frame, plate_frame in zip(video_detection(rtsp_url), number_plate_detection(rtsp_url)):
        #             img_ppe = cv2.imdecode(np.frombuffer(ppe_frame, np.uint8), cv2.IMREAD_COLOR)
        #             img_plate = cv2.imdecode(np.frombuffer(plate_frame, np.uiationt8), cv2.IMREAD_COLOR)

        #             combined_frame = cv2.addWeighted(img_ppe, 0.7, img_plate, 0.3, 0)

        #             _, img_encoded = cv2.imencode('.jpg', combined_frame)
        #             video_placeholder.image(img_encoded.tobytes(), channels="BGR")
